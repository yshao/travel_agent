...
class ItineraryEntry(BaseModel):
    ...
    travel_distance: Optional[str] = Field(None, description="Travel time/distance to the activity. Empty if last activity of day/trip.")
    ...
    @validator('travel_distance', pre=True)
    def parse_travel_distance(cls, v):
        ...
        # if match_min and match_min.group(1):
        #     return str(float(match_min.group(1).replace(',', '')))

        # # Handle "hour travel time"
        # match_hour = re.search(r'(\d[\d,.]*)\s*hour', s)
        # if match_hour and match_hour.group(1):
        #     return str(float(match_hour.group(1).replace(',', '')) * 60)  # Convert hours to minutes

        # # Fallback for just a number
        # match_num = re.search(r'(\d[\d,.]*)', s)
        # if match_num and match_num.group(1):
        #     return str(float(match_num.group(1).replace(',', '')))
...
def parse_itinerary_content(itinerary_content: str) -> List[ItineraryEntry]:
    ...
    for line in lines:
        ...
        if current_day and line.startswith('*'):
            ...
            # 1. Extract and remove cost
            # Try to match cost in parentheses first e.g. ($25)
            cost_chunk_with_paren_pattern = re.compile(r"(\s*\(\s*\$([\d,.]+)(?:\s*per person)?\s*\))")
            cost_match = cost_chunk_with_paren_pattern.search(activity)
            if cost_match:
                cost = cost_match.group(2).strip()
                activity = activity.replace(cost_match.group(1), '', 1)
            else:
                # If not in parens, match just the cost e.g. $25 per person
                cost_chunk_no_paren_pattern = re.compile(r"(\s*\$([\d,.]+)(?:\s*per person)?)")
                cost_match = cost_chunk_no_paren_pattern.search(activity)
                if cost_match:
                    cost = cost_match.group(2).strip()
                    activity = activity.replace(cost_match.group(1), '', 1)
            ...
            if loc_match:
                ...
                activity = activity.replace(loc_match.group(0), '', 1)

            # 3. Extract and remove travel distance
            travel_distance_extract_pattern = re.compile(r"\s*\(([^)]*(?:min|minute|hour)[^)]*)\)")
            travel_match = travel_distance_extract_pattern.search(activity)
            if travel_match:
                travel_distance = travel_match.group(1).strip()
                activity = activity.replace(travel_match.group(0), '', 1)
            ...
            try:
                ...
                itinerary_entries.append(ItineraryEntry(
                    ...
                    cost=None, travel_distance=None
                ))
...
def generate_csv_from_itinerary_entries(itinerary_entries: List[ItineraryEntry]) -> str:
    ...
    for entry in itinerary_entries:
        writer.writerow([
            ...
            str(entry.travel_distance) if entry.travel_distance is not None else ""
        ])
